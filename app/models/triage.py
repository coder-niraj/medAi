...
class TriageResult(Base):
    ...
    session_id = Column(
        UUID(as_uuid=True), ForeignKey("chat_sessions.id"), unique=True, nullable=False
    )
    ...
    result_level = Column(
        Enum("green", "yellow", "red", name="result_level_enum"), nullable=False
    )
    ...
    patient_acted = Column(
        Enum("sought_care", "monitored_at_home", "ignored", name="patient_action_enum"),
        nullable=True,
    )
