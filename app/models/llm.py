...
class LLMTrace(Base):
    ...
    session_id = Column(
        UUID(as_uuid=True), ForeignKey("chat_sessions.id"), unique=True, nullable=True
    )
    ...
    call_type = Column(
        Enum(
            "chat", "triage_completion", "summary", "embedding", name="call_type_enum"
        ),
        nullable=False,
    )
